...
<?php
  if($wyszukiwanie) {
    ...
    $zapytanie = "SELECT * FROM mojekursy WHERE nazwa_konta='$uzytkownik' AND nazwa_kursu='$wyszukiwanie';";
    $rezultat = mysqli_query($polaczenie, $zapytanie);
    $rezultatwynik = mysqli_num_rows($rezultat);
    $array = array();
    echo "<table>";
    echo "<h2> Kursy, do których jestem zapisany: </h2>";
    echo "<tr><td>" . 'Nazwa' . "</td><td>" . 'Ilość godzin' . "</td><td>" . 'Opis Kursu' . "</td><td>" . 'Cena' . "</td><td>" . 'Imie nauczyciela' . "</td><td>" . 'Nazwisko nauczyciela' . "</td><td>" . 'Opis nauczyciela' . "</td><td>" . 'Tryb' . "</td><td>" . 'Termin' . "</td><td>" . 'Potwierdzony' . "</td></tr>";
    if ($rezultatwynik > 0)
    {
     while ($wiersz = mysqli_fetch_assoc($rezultat)) // mysqli_fetch_assoc - daje mi liczbę poszczególnych wierszy w bazie danych na postawie tego selecta na gorze do $zapytanie
     {
       ...
       "</td><td>" . $wiersz['opis_nauczyciela'] . "</td><td>" . $wiersz['nazwa_trybu'] . "</td><td>" . $wiersz['data_terminu'] . "</td><td>" . $wiersz['potwierdzenie'] . "</td></tr>";
       ;
  }
}
echo "</table>";
  ...
  require 'formalogowania/bazadanych.php';
  $zapytanie = "SELECT * FROM mojekursy WHERE nazwa_konta='$uzytkownik';";
  $rezultat = mysqli_query($polaczenie, $zapytanie);
  $rezultatwynik = mysqli_num_rows($rezultat);
  $array = array();
  echo "<table>";
